...
// import CtaClient from "./_components/Service/CtaClient";

// // ─── Dynamic imports with code splitting ──────────────────────────────────────

// /**
//  * BEFORE: Framer Motion was imported at the top of ServicesPage.tsx and loaded
//  *         synchronously with the page bundle — bloating the critical JS path.
//  *
//  * AFTER:  next/dynamic defers each component's JS until after the initial HTML
//  *         has streamed. ssr:false for the hero means the browser-only animation
//  *         APIs never run during server rendering (no hydration mismatch risk).
//  */

// const AnimatedHero = dynamic(() => import("./_components/Service/AnimatedHero"), {
//   ssr: false,
//   loading: () => (
//     // Skeleton holds 100vh height → prevents layout shift (CLS = 0 for hero).
//     <div className="h-screen w-full bg-[#0D0905]" aria-hidden="true" />
//   ),
// });

// const ServicesAccordion = dynamic(
//   () => import("../../components/Service/ServicesAccordion"),
//   {
//     ssr: true, // renders initial HTML server-side for SEO / faster FCP
//     loading: () => <div className="py-24 bg-[#E9DED9]" aria-hidden="true" />,
//   }
// );

// const MethodologySection = dynamic(
//   () => import("@/components/components\Service\Methodologysection.tsx"),
//   {
//     ssr: true,
//     loading: () => <div className="py-12 bg-[#231A15]" aria-hidden="true" />,
//   }
// );
...
//       {/* 1. Hero — ssr:false → no hydration mismatch from browser-only APIs */}
//       <Suspense
//         fallback={
//           <div className="h-screen w-full bg-[#0D0905]" aria-hidden="true" />
//         }
...
//       {/* 2. Services accordion */}
//       <Suspense
//         fallback={<div className="py-24 bg-[#E9DED9]" aria-hidden="true" />}
//       >
//         <ServicesAccordion />
//       </Suspense>
...
//       {/* 3. Methodology */}
//       <Suspense
//         fallback={<div className="py-12 bg-[#231A15]" aria-hidden="true" />}
//       >
//         <MethodologySection />
//       </Suspense>

//       {/* 4. CTA — server markup shell + minimal client island for i18n */}
//       <CtaSection />
//     </main>
//   );
// }

// // ─── CTA section — server component shell ─────────────────────────────────────
// // Only CtaClient (the translated text + button) is a client component.
// // The decorative watermark h2 ships as static HTML — no JS needed.
...
//           {/*
//            * BEFORE: Entire CTA was inside the "use client" ServicesPage.
//            * AFTER:  Only the translated content is a client island.
//            *         The section's chrome (bg, padding, container) is server HTML.
//            */}
//           <CtaClient />
//         </div>
//       </div>
...
//       {/*
//        * Decorative watermark — pure HTML, zero JS, no hydration needed.
//        * aria-hidden keeps it out of the accessibility tree.
//        */}
//       <h2
//         className="absolute bottom-0 left-1/2 -translate-x-1/2 text-[12vw] font-bold text-[#8C827A]/15 tracking-wide font-computerist whitespace-nowrap leading-none select-none"
//         aria-hidden="true"
//       >
//         Martini{" "}
//         <span className="text-[10vw]">
//           S
//           <span className="font-Hind_Guntur text-[60px] md:text-[12vw]">o</span>
//           lis
//         </span>
//       </h2>
//     </section>
...
import { Suspense } from "react";
import dynamic from "next/dynamic";
import CtaClient from "@/components/Service/CtaClient";

// 1. المكونات التي تحتاج تعطيل SSR (التي تعتمد على window أو Framer Motion بشكل كثيف)
const AnimatedHero = dynamic(() => import("@/components/Service/AnimatedHero"));

// 2. المكونات العادية: يفضل استيرادها بشكل عادي إذا كانت تدعم الـ SSR 
// أو استخدام dynamic بدون loading إذا كنت ستغلفها بـ Suspense يدوياً
const ServicesAccordion = dynamic(() => import("@/components/Service/ServicesAccordion"));
const MethodologySection = dynamic(() => import("@/components/Service/MethodologySection"));

export default function ServicesPage() {
  return (
    <main className="min-h-screen">
      {/* Hero Section */}
      <Suspense fallback={<div className="h-screen w-full bg-[#0D0905]" />}>
        <AnimatedHero />
      </Suspense>

      {/* Services Section */}
      <Suspense fallback={<div className="py-24 bg-[#E9DED9]" />}>
        <ServicesAccordion />
      </Suspense>

      {/* Methodology Section */}
      <Suspense fallback={<div className="py-12 bg-[#231A15]" />}>
        <MethodologySection />
      </Suspense>
  ...
  return (
    <section className="py-28 text-center relative overflow-hidden bg-[#E9DED4]">
      ...
      >
        ...
        <span className="text-[10vw]">
          S
          <span className="font-Hind_Guntur">o</span>
          lis
        </span>
      </h2>
    </section>
