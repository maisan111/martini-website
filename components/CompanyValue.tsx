...
// ✅ Defined once at module level — never recreated, not part of render cycle
const HEXAGON_CLIP = 'polygon(50% 0, 100% 25%, 100% 75%, 50% 100%, 0 75%, 0 25%)';
const hexStyle = { clipPath: HEXAGON_CLIP } as const;

// ✅ Animations defined outside component — stable object references, no GC pressure
const cardVariants = {
...
const hoverVariants = { hover: { y: -10 } };
...
// ✅ React.memo: HexagonCard only re-renders if its own item/index/isAr changes
const HexagonCard = React.memo(function HexagonCard({
    item,
    index,
    isAr,
}: {
    ...
    const textClass = isAr
        ? "font-Camel tracking-normal"
        : "font-Sanseriffic tracking-[0.4em]";

    const titleClass = isAr
        ? "font-Camel text-[24px]"
        : "font-Sanseriffic tracking-[0.25em] text-[20px]";

    const detailsClass = isAr
        ? "font-Camel text-[21px]"
        : "font-Sanseriffic tracking-widest text-[18px]";

    return (
        <motion.div
            // ✅ viewport={{ once: true }} was already correct — keep it
            custom={index}
            initial="hidden"
            whileInView="visible"
            whileHover="hover"
            ...
            className="relative w-[330px] h-[370px] md:w-[370px] md:h-[400px] group cursor-default"
        >
            {/* Main card shape — style object now references stable constant */}
            <div
                className="absolute inset-0 w-full h-full transition-all duration-700 ease-out bg-[#F3EBE1]/40 backdrop-blur-sm group-hover:bg-[#EADDCF]/60"
                style={hexStyle}
            >
                {/* Inner border */}
                <div
                ...
                {/* Content */}
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-8 z-10">
                    <span className={`text-[#8C827A] text-[13px] font-bold mb-4 opacity-60 group-hover:opacity-100 transition-opacity ${textClass}`}>
                        0{item.id}
                    </span>

                    <h3 className={`text-[#4A3F35] font-semibold uppercase mb-5 group-hover:text-[#4A3F35] transition-colors duration-500 ${titleClass}`}>
                        {item.title}
                    </h3>

                    <div className="h-[1px] w-32 bg-gradient-to-r from-transparent via-[#D3B79C]/40 to-transparent mb-8" />

                    <p className={`text-[#8C827A] px-2 max-w-70 leading-relaxed font-light group-hover:text-[#4A3F35] transition-colors duration-500 ${detailsClass}`}>
                        {item.details}
                    </p>
                </div>
...
const CompanyValues = () => {
    ...
    // ✅ Memoized values array — only rebuilt when language changes
        ...
        >
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className={`mb-24 flex flex-col ${isAr ? 'items-start text-right' : 'items-start text-left'}`}>
                    ...
                    <h2 className={`text-4xl md:text-6xl font-extralight text-[#4A3F35] mb-8 ${isAr ? 'font-Camel' : 'font-Sanseriffic tracking-widest'}`}>
                        {t('company_values.title_main')}{' '}
                        <span className="font-bold text-[#8C827A]">{t('company_values.title_bold')}</span>
                    </h2>
